...
/**
 * The backup: keeps WorkManager's schedule in step with the settings. Nothing is scheduled until the user has allowed the network and turned the backup on, and turning either
 * off cancels everything. A run happens only when Android says the conditions the user chose hold (the network they allowed, the battery not low, charging if they asked for it).
 */
@Singleton
class HomeServerBackup @Inject constructor(
    ...
    /** Applies the schedule for the settings as they are, and keeps it in step with them; called once when the app starts. */
    fun start() {
    ...
    private fun apply(current: BackupSettings) {
        if (!current.active || BackupSettings.normalizedUrl(current.serverUrl).isEmpty() || !settings.hasCredential()) {
