...
/** A square-friendly thumbnail of [item], sized by the layout it is placed in. */
@Composable
fun MediaThumbnail(
    item: MediaItem,
    modifier: Modifier = Modifier,
    contentScale: ContentScale = ContentScale.Crop,
) = MediaThumbnail(item.id, item.isVideo, item.modifiedAt, modifier, contentScale)
...
fun MediaThumbnail(
    ...
    val request = remember(id, modifiedAt) {
        ImageRequest.Builder(context)
            .data(MediaThumbnailData(mediaContentUri(id, isVideo), modifiedAt))
            .build()
    }
    AsyncImage(
        model = request,
